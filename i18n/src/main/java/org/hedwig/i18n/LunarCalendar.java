 ...
 * <br>
 */
public final class LunarCalendar extends GregorianCalendar {

	private static final long serialVersionUID = 7690710339780076477L;
	/** 农历年 */
     ...
     * @param d
     */
    public LunarCalendar(boolean isLunarYear, int year, int month, int day) {
        if (isLunarYear) {
            set(CHINESE_YEAR, year);
            set(CHINESE_MONTH, month);
            set(CHINESE_DATE, day);
        } else {
            set(year, month, day);
        }
    }
    ...
    private void computeGregorianFields() {
        ...
        if (dateint < 19001111) { // 太小
            ...
            int days = LunarCalendar.daysInChineseMonth(y, m);
            if (days == 0) {
                m = -m;
                days = LunarCalendar.daysInChineseMonth(y, m);
            }
            if (d > days) {
                d = days;
            }
